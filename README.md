Интерпретатор lua-подобного языка программирования. Язык называется ITMOScript
...
Существует отдельная папки с 9 категориями тестов, которые должны заполнятся по очереди. Более подрбную информацию про то, как устроены тесты и что они тестируют можно найти внутри папки tests в документе info

## Примеры

В директории `tests` есть ряд примеров программ написанных на ITMOScript
